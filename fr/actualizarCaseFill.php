<? 
	...
	if($periodo){
		...
		$queries['Limpiando Case Fill'] = "DELETE FROM fr_con WHERE c0 LIKE 'Case Fill' AND per = '" . $_SESSION['periodo'] . "'";

		$queries['Concentrado Case Fill'] = "insert into fr_con(
						select '" . $_SESSION['periodo'] . "','Case Fill', c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, c14, c15, c16, c17, c18, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''
						from fr_cas_fil
					);";
		...
		$queries['Actualizando Sku restantes'] = "UPDATE fr_con
						...
						set fr_con.c14 = fr_mae_mat.des,
							...
							fr_con.matGro5 = fr_mae_mat.matGro5,
							fr_con.matGro5Des = fr_mae_mat.matGro5Des;";
		...
		$resumenUrl .= registrarEvento($conexion, $_SESSION['periodo'], 'Admin', 'fr_cas_fil');
