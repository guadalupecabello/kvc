<? 
	...
	$queryPeriodos = "SELECT per FROM periodos;";

	$periodos = obtenerQueryToArray($conexion, $queryPeriodos);
        			...
        			<select id="selectInicio">
        				...
        				<? 
        					...
        					foreach ($periodos as $indice => $periodo) {
        						...
        						if($periodo['per'] == $_SESSION['periodo']){
        							$seleccionado = " selected ";
        						}
        				?>
    						
    							<option value=<?= $periodo['per'] ?> <?= $seleccionado ?>> <?= $periodo['per'] ?> </option>
...
<script>
	...
	function crearQuery(){
		...
		var selectQuerySumas = 'SELECT \'Total\', ';
		var campos = '';
		...
		return queries;
			...
			$('#exportar').click(function(){
				...
				$('#queryExportar').val(queries.query);
				$('#queryExportarSumas').val(queries.querySumas);
				$('#forma').submit();

				// $('#consola').html('<hr>' + queries.query  + '<hr>' + queries.querySumas);
